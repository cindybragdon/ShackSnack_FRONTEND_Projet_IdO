...
// Fonction pour enregistrer le JWT dans AsyncStorage
export async function setUserDevice(device) {
    try {
        if (device) {
            const deviceString = JSON.stringify(device);
            await AsyncStorage.setItem('device', deviceString);
        } else {
            throw new Error('Impossible de définir un device null ou undefined');
        }
    } catch (error) {
        console.error("Erreur lors de l'enregistrement du des animaux du user :", error);
    }
...
export async function getUserDevice() {
    try {
        ...
        console.error('Erreur lors de la récupération du device du user :', error);
        return null;
    }
}
...
api.interceptors.request.use(
    async (config) => {
        const token = await getToken();
        console.log('Tocken récupéré :', token); 
        if (token) {
            config.headers.Authorization = `Bearer ${token}`;
        }
        return config;
...
export async function signIn(email, password) {
    try {
        ...
        await setUserDevice(userAuth.data.devices[0]);
...
export async function getAllFeedingLogsOfUser(id) {
    try {
        ...
        if (userAuth.status !== 200) throw new Error('Échec de get des feeding logs du user');
        await setToken(userAuth.data.token);
        return userAuth.data;
    } catch (error) {
        console.error('Erreur lors de l\'inscription :', error);
...
// Function pour updater les informations du user
...
export async function createFeedingLog(animalId, deviceId, feeding_food_type, feeding_quantity) {
    try {
        ...
        const now = new Date();
        const feedingLogData = { userId:userid, animalId:animalId, deviceId:deviceId, feeding_date:now, feeding_food_type:feeding_food_type, feeding_quantity:feeding_quantity };
...
export async function getAllUsers() {
    try {
        ...
        if (userAuth.status !== 200) throw new Error('Échec du get les users');
        return userAuth.data;
    } catch (error) {
...
export async function updateUser(userId, newUsername, newFirstName, newLastName, newEmail) {
    try {
        console.log("Modification du nom d'utilisateur pour le userId:", userId, "avec le nom d'utilisateur:", newUsername);
        ...
        console.log("Nom d'utilisateur modifié avec succès:", response.data);
...
export async function deleteUserById(userId) {
    try {
        console.log("Supression de l'utilisateur avec le userId  :", userId);
        ...
        console.log("Succès lors de la suppression de l'utilisateur avec le id:", userId);
        return response.data;
    } catch (error) {
...
// animals, id , form
export async function createSubdocument(userId, subDocName, data) {
    try {
        const userAuth = await api.post(`/users/${userId}/${subDocName}`,data);
